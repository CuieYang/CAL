...
def sp_loss(input, target, weight):
    criterion = nn.NLLLoss(reduction='none').cuda()
    loss = torch.mul(criterion(torch.log(1.0 - input), target)*weight, 1).mean()
    return loss
...
def HDA_UDA(fea, softmax_output, ad_net, coeff=None, cls_weight=None, myloss=Myloss()):
    ad_out = ad_net(fea)
    ...
    #print('type', target_weight, cls_weight)
    target_weight = target_weight * cls_weight
    weight = source_weight / torch.sum(source_weight).detach().item() + \
             target_weight / torch.sum(target_weight).detach().item()
    return myloss(ad_out,dc_target,weight.view(-1, 1)), mean_entropy

def HDA_SSDA(input_list, ad_net, coeff=None, myloss=Myloss(), cls_weight=None, len_share=0):
